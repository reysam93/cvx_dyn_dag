# Code adapted from the CAUSALNEX repository:
# https://github.com/mckinsey/causalnex
# Copyright 2019–2020 QuantumBlack Visual Analytics Limited
# Licensed under the Apache License, Version 2.0 (see full text below)
...
class Dynotears():
    ...
    def fit(self,
        ...
        Args:
            ...
            lamb_W (float): l1 regularization parameter of intra-weights W
            lamb_A (float): l1 regularization parameter of inter-weights A
        ...
        -------
        W : np.ndarray
            Shape (d, d). Estimated intra-slice weighted adjacency.
        A : np.ndarray
            Shape (d, p*d) or (p*d, d) depending on your convention **(document it!)**.
            En tu código de arriba devuelves `a_est` tal cual; si es horizontal, documenta (d, p*d).
        ...
        w_est, a_est = _learn_dynamic_structure(
            X, Xlags, bnds, lamb_W, lamb_A, max_iter, h_tol
        )
    ...
    # TODO: use their original _h function 
    def dagness(self, W):
        """
        ...
        return np.trace(expm(W)) - W.shape[0]
